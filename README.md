...
- 깃으로 관리하는 파일의 4가지 상태
    ...
    - 새로운 Branch를 추가
    - 브렌치는 커밋을 가리킨다(브렌치 -> 커밋)
    - 저장소의 병렬된 버전
    - 메인 분기, 기본 분기에 영향을 주지 않고 자유롭게 작업 가능
    - 원하는 작업일 때 다시 갈라진 브렌치(분기)를 기본 브렌치에 병합
...
- base 브렌치를 기준으로 compare 브렌치를 병합한다.
- 충돌 병합 해결하기
    - 충돌이 일어난 파일을 열어보면 
```
<<<<<< HEAD
...
- vs Code 충돌해결 기능
    - Accept Curent Change (베이스 브렌치의 내용만 남김)
    - Accept Incoming Change (compare 브렌치의 내용만 남김)
